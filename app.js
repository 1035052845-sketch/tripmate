...
let expenses = JSON.parse(localStorage.getItem("tripmate-expenses") || "null") || defaultExpenses;
...
function saveExpenses() {
  localStorage.setItem("tripmate-expenses", JSON.stringify(expenses));
  byId("syncTime").textContent = "刚刚更新";
}
...
function renderMoney() {
  ...
  byId("quickSettlement").textContent = mySettlement
    ? mySettlement.from === "lulu"
      ? `${money(mySettlement.amount)} · 等旅行结束再转也可以`
      : `对方应付你 ${money(mySettlement.amount)}`
    : "目前已经完全平账";
}
...
byId("expenseForm").addEventListener("submit", event => {
  ...
  renderActivities();
  closeExpenseModal();
  showToast("开销已记录，并同步给所有同行朋友");
});

byId("inviteButton").addEventListener("click", () => openSimpleModal("邀请朋友加入", "将旅行码发给朋友，他们加入后就能看到所有最新安排。", `<div class="invite-code">SEOUL630</div><button class="button button-primary full-button" onclick="navigator.clipboard?.writeText('SEOUL630'); document.getElementById('simpleModal').classList.remove('open');">复制旅行码</button>`));
